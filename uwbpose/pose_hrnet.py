...
class PoseHighResolutionNet(nn.Module):
    ...
    def __init__(self, stage_cfg, **kwargs):
        self.inplanes = 64
        extra = stage_cfg2
    ...
    def forward(self, x):
        ...
        #print(x.shape)  # torch.Size([1, 64, 64, 64])
        x = self.relu(x)
        #rint(x.shape)  # torch.Size([1, 64, 64, 64])
        x = self.conv2(x)
        ...
        coords, maxvals = get_max_preds(x.cpu().detach().numpy())
...
def get_pose_hrnet(**kwargs):
    
    model = PoseHighResolutionNet(stage_cfg, **kwargs)
