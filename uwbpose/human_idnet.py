...
class Class_ResNet(nn.Module):    
    def __init__(self, block, layers):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv1d(1, 64, kernel_size=10, stride=3, padding=4, bias=False),
            nn.BatchNorm1d(self.inplanes, momentum=BN_MOMENTUM),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(64, 128, kernel_size=3, stride=3, padding=0, bias=False),
            ...
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(256, 512, kernel_size=5, stride=5, padding=0, bias=False),
            nn.BatchNorm1d(512, momentum=BN_MOMENTUM),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(512, 10, kernel_size=10, stride=1, padding=0, bias=False),
        )
...
#################UNet########################
    def __init__(self, block, layers):
        ...
        self.unpool4 = nn.ConvTranspose1d(in_channels = 512, out_channels = 512, kernel_size = 2, stride = 2, padding = 0) # (512, 224)
        self.decoding4_1 = conv1(2*512, 512)
        self.upto3 = conv1(512, 256)
        
        self.unpool3 = nn.ConvTranspose1d(in_channels = 256, out_channels = 256, kernel_size = 2, stride = 2, padding = 0) # (256, 448)
        self.decoding3_1 = conv1(2*256, 256)
        self.upto2 = conv1(256, 128)
        
        self.unpool2 = nn.ConvTranspose1d(in_channels = 128, out_channels = 128, kernel_size = 2, stride = 2, padding = 0) # (128, 896)
        self.decoding2_1 = conv1(2*128, 128)
        self.upto1 = conv1(128, 64)
        
        self.unpool1 = nn.ConvTranspose1d(in_channels = 64, out_channels = 64, kernel_size = 2, stride = 2, padding = 0) # (64, 1792)
        self.decoding1_1 = conv1(2*64, 64) #(64, 1792)
        self.out = nn.Conv1d(64, 1, 9, 1, 4, bias = False)
    
    def forward(self, x):
        x = self.encoding1_1(x) #(1, 1792) -> (64, 1792)
        encoding1 = self.encoding1_2(x)
        #print("layer1", encoding1.shape)
        x = self.downto2(encoding1) #(64, 1792) -> (64, 896)
        
        x = self.encoding2_1(x) #(64, 896) -> (128, 896)
        encoding2 = self.encoding2_2(x)
        #print("layer2", encoding2.shape)
        x = self.downto3(encoding2) #(128, 896) -> (128, 448)
        
        x = self.encoding3_1(x) #(128, 448) -> (256, 448)
        encoding3 = self.encoding3_2(x)
        #print("layer3", encoding3.shape)
        x = self.downto4(encoding3) #(256, 448) -> (256, 224)
        
        x = self.encoding4_1(x) #(256, 224) -> (512, 224)
        encoding4 = self.encoding4_2(x)
        #print("layer4", encoding4.shape)
        x = self.downto5(encoding4) #(512, 224) -> (512, 112)
        
        x = self.encoding5_1(x) #(512, 112) -> (1024, 112)
        x = self.encoding5_2(x)
        x = self.upto4(x) 
        
        x = self.unpool4(x) #(512, 112) -> (512, 224)
        x = torch.cat([encoding4, x], dim = 1)
        #print("layer up 4", x.shape)
        x = self.decoding4_1(x)
        x = self.upto3(x)
        
        x = self.unpool3(x) #(256, 224) -> (256, 448)
        x = torch.cat([encoding3, x], dim = 1)
        x = self.decoding3_1(x)
        x = self.upto2(x)
        
        x = self.unpool2(x) #(128, 448) -> (128, 896)
        x = torch.cat([encoding2, x], dim = 1)
        x = self.decoding2_1(x)
        x = self.upto1(x)
        
        x = self.unpool1(x) #(64, 896) -> (64, 1792)
...
'''
    def __init__(self, block, layers) :
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.BatchNorm1d(self.inplanes, momentum=BN_MOMENTUM),
            nn.ReLU(inplace=True),
            nn.Conv1d(64, 16, kernel_size=7, stride=1, padding=3, bias=False),
            nn.BatchNorm1d(16, momentum=BN_MOMENTUM),
            nn.ReLU(inplace=True),
            nn.Conv1d(16, 4, kernel_size=7, stride=1, padding=3, bias=False),
            nn.BatchNorm1d(4, momentum=BN_MOMENTUM),
            nn.ReLU(inplace=True),
            nn.Conv1d(4, 1, kernel_size=7, stride=1, padding=3, bias=False),
...
class Discriminator(nn.Module):
    def __init__(self, block, layers) :
        ...
        self.conv1 = nn.Sequential(
            nn.Conv1d(2, 64, kernel_size=10, stride=3, padding=4, bias=False),
            nn.BatchNorm1d(self.inplanes, momentum=BN_MOMENTUM),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(64, 128, kernel_size=3, stride=3, padding=0, bias=False),
            nn.BatchNorm1d(128, momentum=BN_MOMENTUM),
